    ...
    pub fn insert_tab(&mut self, tab: &Arc<Tab>, position: TabInsertPosition) -> usize {
        let idx = match position {
            ...
            TabInsertPosition::AfterActive => {
                usize::min(self.active_tab_idx.saturating_add(1), self.tabs.len())
            }
        };
        self.insert_tab_at_idx(idx, tab);
        idx
    }
...
mod tests {
    ...
    fn insert_after_active_places_new_tab_next_to_active() {
        ...
        assert_eq!(idx, 1);
        assert_eq!(window.get_tab_idx_for_id(a.tab_id()), Some(0));
        assert_eq!(window.get_tab_idx_for_id(n.tab_id()), Some(1));
        assert_eq!(window.get_tab_idx_for_id(b.tab_id()), Some(2));
        assert_eq!(window.get_tab_idx_for_id(c.tab_id()), Some(3));
    ...
    fn append_position_keeps_existing_behavior() {
        ...
        assert_eq!(idx, 3);
        assert_eq!(window.get_tab_idx_for_id(a.tab_id()), Some(0));
        assert_eq!(window.get_tab_idx_for_id(b.tab_id()), Some(1));
        assert_eq!(window.get_tab_idx_for_id(c.tab_id()), Some(2));
        assert_eq!(window.get_tab_idx_for_id(n.tab_id()), Some(3));
        assert_eq!(window.get_active_tab_idx(), 3);
